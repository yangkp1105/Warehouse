...
class HoldemTable(Env):
    ...
    def _distribute_cards(self):
        ...
        for player in self.players:
            ...
            # 只显示自己的手牌（假设你是玩家 0）
            if player.seat == 0:
                log.info(f"玩家 {player.seat} 手牌 {player.cards}，筹码 ${player.stack}")
            else:
                log.info(f"玩家 {player.seat} 手牌 [??, ??]，筹码 ${player.stack}")
    def _distribute_cards_to_table(self, amount_of_cards):
        ...
        table_cards_str = str(self.table_cards)
        table_cards_str = table_cards_str.replace("'H'", "'♥'").replace("'S'", "'♠'").replace("'D'", "'♦'").replace("'C'", "'♣'")
        log.info(f"公共牌: {table_cards_str}")
    ...
    def render(self, mode='human'):
        ...
        for i in range(len(self.players)):
            ...
            self.viewer.text(f"{self.players[i].name}", x - 60, y - 15,
                             font_size=10,
                             color=WHITE)
            cards_str = str(self.players[i].cards)
            cards_str = cards_str.replace("'H'", "'♥'").replace("'S'", "'♠'").replace("'D'", "'♦'").replace("'C'", "'♣'")
            # 如果是 AI 玩家且不是你自己，可以隐藏手牌
            if i != 0:  # 假设你是玩家 0
                cards_str = "[??, ??]"
            self.viewer.text(f"玩家 {self.players[i].seat}: {cards_str}", x - 60, y,
                            font_size=10,
                            color=WHITE)
            ...
            table_cards_str = str(self.table_cards)
            table_cards_str = table_cards_str.replace("'H'", "'♥'").replace("'S'", "'♠'").replace("'D'", "'♦'").replace("'C'", "'♣'")
            self.viewer.text(f"{table_cards_str}", screen_width / 2 - 40, screen_height / 2, font_size=10,
                            color=WHITE)
